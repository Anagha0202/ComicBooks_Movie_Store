...
ACCEPT DATEAFTER CHAR PROMPT 'Enter the Date(YYYY-MM-DD) after which orders have to be displayed: '
...
SPOOL CustomerOrderAfterDateOutput.txt 

BREAK ON CUSTID ON CUSTTYPE ON NAME ON PHONE ON STREET ON CITY ON ZIPCODE ON ORDERCOST ON TAX ON DISCOUNT ON SHIPPINGFEE ON FINALCOST SKIP 1 
...
AND dateoforder BETWEEN TO_dATE('&DATEAFTER', 'yyyy-mm-dd') and SYSDATE
AND dateoforder is not null
ORDER BY CO.ORDERID, DATEOFORDER;
